...
const Navigation = () => {
  ...
  return (
    <div className='flex relative'>
        <div className={`bg-white cursor-pointer z-40 lg:rounded-md lg:w-[90%] right-0 md:w-[50%] w-[70%] lg:h-12 h-full lg:mt-5 mt-0 m-auto lg:py-1 px-5 py-10 lg:px-2 gap-9 lg:gap-1 fixed lg:sticky lg:flex lg:flex-row justify-between md:items-center items-start lg:items-center drop-shadow-2xl shadow-2xl transition-all duration-500 ${isMenuOpen ? 'right-0' : 'right-[-100%]'}`}>
        ...
        <div className='absolute right-8 z-50 top-16 cursor-pointer lg:hidden' onClick={toggleMenu}>
                {isMenuOpen 
                    ?
                    (<LiaWindowCloseSolid  size={30} className='text-black z-50' />)
                    :
                    (<HiMenuAlt3 size={30} className='text-white z-50' />)
                }
            </div>
